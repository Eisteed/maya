...
class RigCurveTool(object):
    ...
    def wireOnly(self,*args,**kwargs):
        ...
        if not cmds.objExists(rigNode):
            raise RuntimeError("{} not found in scene, rig not built yet?".format(rigNode))
        allKids = cmds.listRelatives(rigNode,ad=True)
        if not cmds.objExists(wireCrv) and not wireCrv in allKids:
            raise RuntimeError("wire curve {} not found under {}, wire curve deleted or not rigged?".format(wireCrv,rigNode))
    ...
    def doIt(self,*args,**kwargs):
        ...
        if not crv or not cmds.objExists(crv):
            raise RuntimeError("{} not found in scene".format(crv))
    ...
    def rigFromCurve(self,crv,numSpans=8,numJoints=10,numCtrls=5,stripWidth = 1.0,ctrlWidth=2.0,geo=None,uMin=0.0,uMax=1.0):
        ...
        for i in range(numJoints):
            ...
            jnt = cmds.joint(p=(0,0,0),n=crv + "_driverJoint{:02d}".format(i)) 
            locator = cmds.spaceLocator(n=crv + "driverLoc{:02d}".format(i))[0]
        ...
        for i in range(numCtrls):
            ...
            if i == 0:
                zero,ctrl = self.makeCubeCtrl(crv + "_Ctrl{:02d}".format(i),size=ctrlWidth*1.8)
                ...
                zero,ctrl = self.makeCubeCtrl(crv + "_Ctrl{:02d}".format(i),size=ctrlWidth)
    ...
    def hideChannels(self,obj,lock=False):
        ...
        for attr in ('s','r','t'):
            for axis in ('x','y','z'):
                cmds.setAttr(obj + ".{}{}".format(attr,axis), keyable=False,channelBox=False,lock=lock)
